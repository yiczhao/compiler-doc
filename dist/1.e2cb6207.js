...
/***/ function(module, exports) {
	...
	exports.default = [{
	  ...
	  subMenu: [{
	    ...
	    { name: 'Pagination 分页器', icon: 'icon', link: 'page' }, { name: '下拉选择', icon: 'icon', link: 'dropchoose' }, { name: '弹出层选择', icon: 'icon', link: 'dialogchoose' }, { name: '楼层选择', icon: 'icon', link: 'floorchoose' }, { name: '根据不同用户显示不同表格', icon: 'icon', link: 'addtable' }]
	  }, {
	    ...
	    subMenu: [{ name: 'Dialog 对话框', icon: 'icon', link: 'dialog' }, { name: 'Modal 模态框', icon: 'icon', link: 'modal' }, { name: 'Image 查看', icon: 'icon', link: 'image' }]
	  }]
	}, {
	  name: '日志 Log',
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (false) {(function () {  module.hot.accept()
	  ...
	  hotAPI.install(require("vue"), false)
	  if (!hotAPI.compatible) return
	  var id = "/Users/zdzdesigner/Documents/KS/KS-FED/compiler-doc/src/doc/components/LoadingMask.vue"
	  if (!module.hot.data) {
	    hotAPI.createRecord(id, module.exports)
	  } else {
	    hotAPI.update(id, module.exports, __vue_template__)
